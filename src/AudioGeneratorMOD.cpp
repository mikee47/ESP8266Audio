...
static const uint8_t sine[64] PROGMEM = {0,   24,  49,  74,  97,  120, 141, 161, 180, 197, 212, 224, 235, 244, 250, 253,
...
bool AudioGeneratorMOD::LoadPattern(uint8_t pattern)
{
	...
	for(row = 0; row < ROWS; row++) {
		for(channel = 0; channel < Mod.numberOfChannels; channel++) {
			...
			for(i = 1; i < 37; i++)
				if(amigaPeriod > ReadAmigaPeriods(i * 8) - 3 && amigaPeriod < ReadAmigaPeriods(i * 8) + 3)
...
bool AudioGeneratorMOD::ProcessRow()
{
	...
	for(channel = 0; channel < Mod.numberOfChannels; channel++) {
		...
		if(note != NONOTE) {
			...
			Player.amigaPeriod[channel] =
				ReadAmigaPeriods(note + Mod.samples[Player.lastSampleNumber[channel]].fineTune);
		...
		if(note != NONOTE ||
		   (Player.lastAmigaPeriod[channel] && effectNumber != VIBRATO && effectNumber != VIBRATOVOLUMESLIDE &&
			...
			case ARPEGGIO:
				if(effectParameter)
					...
					case 0:
						...
						break;
					case 1:
						tempNote = Player.lastNote[channel] + effectParameterX * 8 +
								   Mod.samples[Player.lastSampleNumber[channel]].fineTune;
						if(tempNote < 296)
							Mixer.channelFrequency[channel] = Player.amiga / ReadAmigaPeriods(tempNote);
						break;
					case 2:
						tempNote = Player.lastNote[channel] + effectParameterY * 8 +
								   Mod.samples[Player.lastSampleNumber[channel]].fineTune;
						if(tempNote < 296)
							Mixer.channelFrequency[channel] = Player.amiga / ReadAmigaPeriods(tempNote);
						break;
...
void AudioGeneratorMOD::GetSample(int16_t sample[2])
{
	...
	sumR = 0;
	for(channel = 0; channel < Mod.numberOfChannels; channel++) {
		if(!Mixer.channelFrequency[channel] || !Mod.samples[Mixer.channelSampleNumber[channel]].length)
			continue;
		...
		samplePointer =
			Mixer.sampleBegin[Mixer.channelSampleNumber[channel]] + (Mixer.channelSampleOffset[channel] >> DIVIDER);
		...
		if(Mixer.sampleLoopLength[Mixer.channelSampleNumber[channel]]) {
			if(samplePointer >= Mixer.sampleLoopEnd[Mixer.channelSampleNumber[channel]]) {
				Mixer.channelSampleOffset[channel] -= Mixer.sampleLoopLength[Mixer.channelSampleNumber[channel]]
													  << DIVIDER;
		...
		current =
			FatBuffer.channels[channel][(samplePointer - FatBuffer.samplePointer[channel]) /*& (FATBUFFERSIZE - 1)*/];
		next = FatBuffer
				   .channels[channel][(samplePointer + 1 - FatBuffer.samplePointer[channel]) /*& (FATBUFFERSIZE - 1)*/];
